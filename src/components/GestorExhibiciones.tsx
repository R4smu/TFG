...
export default function GestorExhibiciones({ pelicula, onClose }: GestorExhibicionesProps) {
  ...
  const cargarDatos = async () => {
    ...
    const { data: dataExhib } = await supabase
      .from('exhibicion')
      .select('idexhibicion, fecha, horainicio, preciobase, sala(nombresala)')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 dark:bg-black/80 backdrop-blur-sm transition-colors duration-300">
      <div className="bg-white dark:bg-gray-900 w-full max-w-3xl max-h-[90vh] flex flex-col rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-2xl animate-fade-in transition-colors">
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900 z-10 shrink-0 transition-colors">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 w-8 h-8 rounded-full flex items-center justify-center transition-colors">✕</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 transition-colors">
            ...
            <form onSubmit={crearExhibicion} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
              ...
              <button type="submit" disabled={procesando || salas.length === 0} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold p-2 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                {procesando ? '...' : '+ Añadir'}
              </button>
            </form>
          </div>
                ...
                {exhibiciones.map(ex => (
                  <div key={ex.idexhibicion} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-xl flex justify-between items-center group hover:border-blue-500 dark:hover:border-gray-500 transition-colors shadow-sm">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 transition-colors">{ex.sala.nombresala} • {ex.preciobase}€</p>
                    </div>
                    <button 
                      onClick={() => borrarExhibicion(ex.idexhibicion)} 
                      className="text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-500 dark:hover:bg-red-500/10 p-2 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                      title="Borrar sesión"
                    >
